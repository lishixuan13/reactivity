    ...
    ['push', 'pop', 'shift', 'unshift', 'splice'].forEach((key) => {
        instrumentations[key] = function (...args) {
            ...
            if (oldLen !== newLen) {
                ...
                this.length = newLen;
                trigger(origin, "set" /* SET */, 'length', newLen);
                if (changeArray) {
                    changeArray.forEach(({ type, key, oldVal }) => {
                        trigger(origin, type, formatIntegerKey(key), type === "delete" /* DELETE */ ? undefined : origin[key]);
                    });
                }
            }
            resetTracking();
...
function del(target, key) {
    ...
    delete target[key];
    delete origin[key];
    trigger(origin, "delete" /* DELETE */, formatIntegerKey(key), undefined);
}
