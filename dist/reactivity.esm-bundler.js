import { extend, isArray, isMap, isIntegerKey, isSymbol, hasOwn, isObject, hasChanged, makeMap, capitalize, toRawType, isPlainObject, def as def$1, isFunction, NOOP } from '@vue/shared';
...
function createGetter(isReadonly = false, shallow = false) {
    return function get(target, key, receiver) {
        ...
        if (!isReadonly && targetIsArray && hasOwn(arrayInstrumentations$1, key)) {
            return Reflect.get(arrayInstrumentations$1, key, receiver);
        }
        const res = Reflect.get(target, key, receiver);
        if (isSymbol(key) ? builtInSymbols$1.has(key) : isNonTrackableKeys(key)) {
            return res;
        }
        if (!isReadonly) {
...
function ownKeys(target) {
    ...
    if (shouldTrack) {
        ...
        track(origin, "iterate" /* ITERATE */, isArray(origin) ? 'length' : ITERATE_KEY);
    }
    return target;
}
...
function del(target, key) {
    ...
    if (isReadonly(target) && (process.env.NODE_ENV !== 'production')) {
        console.warn(`Delete operation on key "${String(key)}" failed: target is readonly.`, target);
        return;
    }
    if (!isReactive(target)) {
...
export { EffectScope, ITERATE_KEY, ReactiveEffect, computed, customRef, defProxyRef, deferredComputed, del, disableProxy, effect, effectScope, enableProxy, enableTracking, get, getCurrentScope, has, isProxy, isReactive, isReadonly, isRef, markRaw, onScopeDispose, ownKeys, pauseTracking, proxyRefs, reactive, readonly, ref, resetProxy, resetTracking, set, shallowReactive, shallowReadonly, shallowRef, stop, toRaw, toRef, toRefs, track, trigger, triggerRef, unref };
