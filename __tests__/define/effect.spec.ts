import { del, set, has, ownKeys } from '../../src/defObserver'
import {
  reactive,
  effect,
  stop,
...
describe('reactivity/effect', () => {
  ...
  it('should observe iteration', () => {
    ...
    effect(() => (dummy = ownKeys(list).join(' ')))
  ...
  it('should observe enumeration', () => {
    let dummy = 0
    const numbers = reactive<any>({ num1: 3 })
    effect(() => {
      dummy = 0
      for (const key in ownKeys(numbers)) {
        dummy += numbers[key]
      }
    })
  ...
  it('should handle deep effect recursion using cleanup fallback', () => {
    ...
    for (let i = 1; i < 40; i++) {
      ;((index) => {
        const fx = effect(() => {
          set(results, index, results[index - 1] * 2)
        })
  ...
  it('should observe json methods', () => {
    ...
    effect(() => {
      dummy = JSON.parse(JSON.stringify(ownKeys(obj)))
    })
    set(obj, 'a', 1)
